...
def angles_approx_equal(v1, v2, tol=0.001):
    ...
    return approx_equal(v1, v2, tol) or approx_equal(v1 + 2*math.pi, v2, tol) or approx_equal(v1, v2 + 2*math.pi, tol)

def assert_equal_pose(p1, p2, tol=0.001):
    assert (
        approx_equal(p1.x, p2.x, tol) and
        approx_equal(p1.y, p2.y, tol) and
        angles_approx_equal(p1.theta, p2.theta, tol)
    ), f"{p1} != {p2}"

def test_reduce_angle():

    assert approx_equal(reduce_angle(.5*math.pi), .5*math.pi)
    assert approx_equal(reduce_angle(2.5*math.pi), .5*math.pi)
    assert approx_equal(reduce_angle(1000.5*math.pi), .5*math.pi)

    assert approx_equal(reduce_angle(1.5*math.pi), -.5*math.pi)
    assert approx_equal(reduce_angle(3.5*math.pi), -.5*math.pi)
    assert approx_equal(reduce_angle(1001.5*math.pi), -.5*math.pi)
...
def test_pose_from_dist_turn_in_place():
    ...
    assert_equal_pose(
        pose_from_wheel_distances(-2*math.pi, 2*math.pi, 2.0),
        Pose2D(0.0, 0.0, 0.0))

    # Rotate half-turn in place to right
    assert_equal_pose(
        pose_from_wheel_distances(-math.pi, math.pi, 2.0),
