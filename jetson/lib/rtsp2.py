...
def run_server():
    ...
    raw_writer = create_video_channel_writer(
        svr.mount_channel("/raw"),
        "appsrc is-live=true ! videoconvert ! video/x-raw, format=BGRx",
        fps, (width, height))
        
    processed_writer = create_video_channel_writer(
        svr.mount_channel("/processed"),
        "appsrc is-live=true ! videoconvert ! video/x-raw, format=BGRx",
        fps, (width, height))
    ...
    while True:
        ...
        processed_writer.write(vision.process_frame(frame))
