...
def assign_tags_to_pages(tag_ids, tags_per_page, repeat):
    if repeat:
        return [[tag_id] * tags_per_page for tag_id in tag_ids]
    else:
        return [
            list(g)
...
def scale_images(tags_root, tag_ids, scale):
    ...
    filenames = [
        f"{tags_root}/tagStandard41h12/tag41_12_{tag_id:05}.png" for tag_id in tag_ids
    ]
...
class PageLayout:
    ...
    def scale(self):
        ...
        max_cell_height = (
            self.height - (self.rows * TEXT_HEIGHT) - (self.rows - 1) * MIN_MARGIN
        ) // self.rows
...
def main():
    ...
    parser.add_argument(
        "--tags-root",
        help="Path to directory where apriltag-imgs repo is cloned. Uses APRILTAG_IMGS variable otherwise.",
        default=os.getenv("APRILTAG_IMGS"),
    )
    parser.add_argument(
    ...
    if args.tags_root is None:
        sys.exit(
            "Root of apriltag-imgs repo must be specified using --tags-root or APRILTAG_IMGS variable."
        )
    ...
    tmp_dir = tempfile.mkdtemp()
    for (pageno, page_tags) in enumerate(
        assign_tags_to_pages(tag_ids, args.columns * args.rows, args.repeat)
    ):
